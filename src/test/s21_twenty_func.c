...
START_TEST(twenty_func_check) {
  ...
  str = s21_memchr(src, '4', 10);  // MEMCHR
  str1 = memchr(src, '4', 10);
  ck_assert_ptr_eq(str, str1);

  // char src0[15] = "12345678120";
  // char dst0[15] = "1234567890";
  // ck_assert_int_eq(s21_memcmp(src, dst, 10), memcmp(src, dst, 10));     //
  // MEMCMP ck_assert_int_eq(s21_memcmp(src0, dst0, 10), memcmp(src0, dst0,
  // 10)); // MEMCMP
  ...
  char src2[15] = "1234567890";  // MEMMOVE
  char dst2[15] = "1234567890";
  memmove(&src2[4], &src2[3], 3);
  s21_memmove(&dst2[4], &dst2[3], 3);
  ck_assert_str_eq(dst2, src2);

  char srco[15] = "1234567890";  // MEMMOVE
  char dsto[15] = "1234567890";
  memmove(&srco[4], &srco[3], 3);
  s21_memmove(&dsto[4], &dsto[3], 3);
  ck_assert_str_eq(dst2, src2);

  char src3[15] = "1234567890";  // MEMSET
  char dst3[15] = "1234567890";
  memset(src3, '1', 10);
  s21_memset(dst3, '1', 10);
  ck_assert_str_eq(dst3, src3);

  char src4[15] = "1234567890";  // STRCAT
  ...
  char src5[15] = "1234567890";  // STRNCAT
  ...
  char dst6[15] = "1234567890";  // strchr
  char ch = '7';
  char *qwe;
  char *qwe1;
  ...
  char src7[15] = "1234567890";  // STRCMP
  char dst7[15] = "1234567890";

  int eq = s21_strcmp(src7, dst7);
  int eq1 = strcmp(src7, dst7);
  ck_assert_int_eq(eq, eq1);

  char src8[15] = "1234567890";  // STRCMP
  ...
  char st3[] = {'q', 'r', 's', 't', 'w', 's', 'a', 's', '\0'};  // STRCSPN
  ck_assert_int_eq(strcspn(st3, "zx"), s21_strcspn(st3, "zx"));

  char st2[] = "this is a test";
  ck_assert_int_eq(strspn(st2, "shit "), s21_strspn(st2, "shit "));

  char sr1c[] = "gora";
  char re1s[] = "";
  ck_assert_int_eq(s21_strcspn(re1s, sr1c), strcspn(re1s, sr1c));

  // for (int i = 0; i <= 105; i++) // STERERROR
  // {
  ...
  char sr[] = "this_is_a_";
  ck_assert_pstr_eq(strpbrk(sr, "369"), s21_strpbrk(sr, "369"));  // strpbrk
  ...
  char rc[] = "";
  char es[] = "";
  ck_assert_int_eq(s21_strspn(es, rc), strspn(es, rc));

  char c[] = "gora";
  char r[] = "";
  ck_assert_int_eq(s21_strspn(r, c), strspn(r, c));
